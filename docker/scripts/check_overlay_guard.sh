...
#
# Greps every file under docker/ for two forbidden shapes:
#
...
#   2. A COPY (or ADD) instruction whose destination is the bare directory
...
# Exits non-zero and prints every offending line if either shape is found in
# any Dockerfile.
set -euo pipefail
...
echo "check_overlay_guard: OK -- no rm of /opt/frigate/web content, no non-overlay COPY/ADD destination"
